...
fn get_log_directory() -> PathBuf {
    if cfg!(target_os = "macos") {
        // macOS: ~/Library/Logs/PlexPDF
        let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
        PathBuf::from(home).join("Library").join("Logs").join("PlexPDF")
    } else if cfg!(target_os = "windows") {
        // Windows: %APPDATA%\\PlexPDF\\logs
        let appdata = std::env::var("APPDATA").unwrap_or_else(|_| std::env::temp_dir().to_string_lossy().to_string());
        PathBuf::from(appdata).join("PlexPDF").join("logs")
    } else {
        // Linux: ~/.config/PlexPDF/logs
        let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
        PathBuf::from(home).join(".config").join("PlexPDF").join("logs")
    }
}
